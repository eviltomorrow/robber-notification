...
version=v3.0
#
BINARY=robber-notification
GOFILE=cmd/robber-notification.go
...
docker:
	...
	docker build -t registry.cn-hangzhou.aliyuncs.com/eviltomorrow/robber-notification:$(version) .
	@echo "$(CGREEN)=> Build Success!$(CEND)"

# Publish docker
publish:
	@echo "$(CGREEN)=> Publishing for docker ...$(CEND)"
	docker push registry.cn-hangzhou.aliyuncs.com/eviltomorrow/robber-notification:$(version)
